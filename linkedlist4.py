...
class linked_list: # 노드들의 연결
    ...
    def insert(self, pos, new_node):
        prev = self.get_at(pos-1) # pos 이전의 노드를 prev로 설정
        new_node.next = prev.next # pos+1 을 가리킴
        prev.next = new_node      # pos 와 prev 를 연결시켜줌
        self.node_count += 1

    # 삽입 위치가 맨 앞, 맨 끝일 때 처리
    def insert2(self, pos, new_node):
        # pos 예외 범외 지정 
    ...
    def delete2(self, pos):
        ...
        if self.node_count == 1 & pos == 1: # 유일한 리스트
            self.head = None
            self.tail = None
